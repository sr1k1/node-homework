...
const timeEmitter = require("./events.js");
...
const server = http.createServer({ keepAliveTimeout: 60000 }, (req, res) => {
  ...
  // Modification of code to handle url '/time'
  else if (req.url === "/time") {
    res.writeHead(200, { "Content-Type": "application/json" }); // set request status to successful

    // Retrieve current time using emitter
    const currentTimeObj = {};
    timeEmitter.emit("time", currentTimeObj);

    // Construct response
    res.end(
      JSON.stringify({
        time: currentTimeObj["currentTime"],
      }),
    );
